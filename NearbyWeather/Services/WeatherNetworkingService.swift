...
final class WeatherNetworkingService {
  ...
  func beginListeningNetworkReachability() {
    reachabilityManager?.startListening  { [weak self] status in
        ...
        case .reachable(_): self?.reachabilityStatus = .connected
        }
  ...
  func fetchWeatherInformationForStation(withIdentifier identifier: Int, completionHandler: @escaping ((WeatherDataContainer) -> Void)) {
    ...
    AF.request(Constants.Urls.kOpenWeatherMapSingleStationtDataRequestUrl(with: apiKey, stationIdentifier: identifier),
      ...
      .responseData(queue: weatherFetchQueue) { [weak self] response in
        guard let self = self, let data = response.value, response.error == nil else {
            let errorType = (response.response?.statusCode == 401) ?
                ErrorDataDTO.ErrorType(value: .unrecognizedApiKeyError) :
                ErrorDataDTO.ErrorType(value: .httpError)
  ...
  func fetchBulkWeatherInformation(completionHandler: @escaping (BulkWeatherDataContainer) -> Void) {
    ...
    AF.request(Constants.Urls.kOpenWeatherMapMultiStationtDataRequestUrl(with: apiKey,
      ...
      .responseData(queue: weatherFetchQueue) { [weak self] response in
        guard let self = self, let data = response.value, response.error == nil else {
            let errorType = (response.response?.statusCode == 401) ?
                ...
                ErrorDataDTO.ErrorType(value: .httpError)
            let errorDataDTO = ErrorDataDTO(errorType: errorType, httpStatusCode: response.response?.statusCode)
            return completionHandler(BulkWeatherDataContainer(errorDataDTO: errorDataDTO,
                                                              weatherInformationDTOs: nil))
        }
